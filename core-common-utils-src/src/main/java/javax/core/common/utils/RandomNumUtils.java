...
/**
 * 产生一个 随机数,并生成 验证码图片,(支持中文)
 * 此类使用简单，操作方便，可扩展性强
 * @author Tanyongde
 *
 */
public class RandomNumUtils {   
    protected ByteArrayInputStream image;// 图像
	protected String str;// 验证码

	// 随机类,用于产生随机数
	protected Random random = new Random();

	protected boolean rotate_able; // 是否扭曲

	// 随机码在以下字符中随机抽取,数字1、小写l、大写O、小写o、数字0 这些难于辨别的字符被剔除
    ...
    protected int code_length;  //产生几位随机数
    
    private static RandomNumUtils randomNumUtil;
    
    //禁用构造方法,实现单例
    protected RandomNumUtils(){}
   ...
   public static RandomNumUtils instance(int base_code_key,int length){
	   ...
	   randomNumUtil.code_length = (0 == length) ? 4 : length; 
	   randomNumUtil.init();
    ...
    protected void init() {   
        //在内存中创建图象   
        int width = (16 * code_length) + 6, height = 20;   
	 ...
	 * 
	 * @param key
	 *            传入三个常量， EN_CODE, CN_CODE, UNION_CODE
	 * @return
	 */
...
//     font[2] = new Font("Forte", Font.PLAIN, 17); 
...
//     return font[random.nextInt(5)]; 
    	...
    	try {
    		...
    		// return font.deriveFont(Font.PLAIN, fontHeight); 返回字体//fontHeight 字体的高度      
   ...
   public static void setBaseCode(String baseCode){
	   RandomNumUtils.base_code = baseCode;
   }
